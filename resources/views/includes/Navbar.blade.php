 <!--=================================== NavBar UpperSection==================================== -->
   ...
   <div class="hidden w-full h-14  items-center justify-between bg-blue-900 px-30 xl:px-40 lg:flex">
    <div class="flex items-center space-x-8 ml-12">
        <a href="https://www.instagram.com/citsaucc?igsh=MWN1Nmd4amVmNWNwOA==" class="text-white hover:text-[#ff6b42]" aria-label="Instagram">
            <i class="fab fa-instagram"></i>
        </a>
        <a href="https://x.com/citsa_ucc?s=11" class="text-white hover:text-[#ff6b42]" aria-label="X">
           <i class="fab fa-x"></i>
        </a>
        <a href="http://facebook.com/dcsitucc" class="text-white hover:text-[#ff6b42]" aria-label="FaceBook ">
            <i class="fab fa-facebook"></i>
        </a>
        <a href="https://www.linkedin.com/company/computer-science-and-information-technology-student-association-ucc/"
           class="text-white hover:text-[#ff6b42]" aria-label="LinkedIn">
    ...
    <div class="flex items-center justify-center text-lg space-x-8">
        <div class="flex items-center space-x-2 hover:text-[#ff6b42]">
            ...
            <a href="tel:+233 (03321) 30096" class="text-white no-underline" aria-label="Phone number">+233 (03321) 30096</a>
        </div>
        <div class="flex items-center space-x-2 hover:text-[#ff6b42] pr-8">
            <i class="fas fa-envelope text-white"></i>
            <a href="mailto:dcsit@ucc.edu.gh" class="text-white no-underline" aria-label="Email">dcsit@ucc.edu.gh</a>
        </div>
    </div>
</div>
  ...
  <nav id="navbar" class="hidden lg:flex flex-col lg:flex-row ">
      ...
      <ul class="flex flex-col lg:flex-row space-y-4 lg:space-y-0  lg:space-x-8 lg:items-center z-50 top-0 lg:z-auto lg:static absolute bg-gray-100  w-[93%] left-5 pl-8  transition-all ease-in duration ">
         <div class="close-btn lg:hidden absolute top-0 right-0 mt-4 mr-4 text-2xl cursor-pointer" onclick="toggleMenu()">
          ...
          <li><a href="/" class="text-lg hover:text-red-700 {{ request()->is('/') ? 'text-red-700' : '' }}"  aria-label="Home">Home</a></li>
          <li><a href="{{route('About')}}" class="text-lg hover:text-red-700 {{ request()->is('About') ? 'text-red-700' : '' }}" aria-label="About">About</a></li>
          <li>
    <a href="#" 
       onclick="event.preventDefault(); toggleSubMenu('programmes');" 
       class="text-lg flex items-center space-x-1 
              {{ request()->is('undergraduate') || request()->is('postgraduate') ? 'text-red-700' : '' }}" aria-label="Programmes">
        <span>Programmes</span>
        <span class="dropdown-symbol 
              {{ request()->is('undergraduate') || request()->is('postgraduate') ? 'text-red-700' : '' }}">
            &#9662;
        </span>
    </a>
    <div class="sub-menu1 absolute hidden bg-white shadow-lg z-50" id="sub-programmes">
        <a href="{{ route('undergraduate') }}" 
           class="block px-4 py-2 hover:bg-gray-100 
                  {{ request()->is('undergraduate') ? 'text-red-700' : '' }}" aria-label="Undergraduate">
            Undergraduate
        </a>
        <a href="{{ route('postgraduate') }}" 
           class="block px-4 py-2 hover:bg-gray-100 
                  {{ request()->is('postgraduate') ? 'text-red-700' : '' }}" aria-label="Postgraduate">
            Postgraduate
        </a>
    </div>
</li>
    ...
    <a href="#" 
       ...
       class="text-lg flex items-center space-x-1 
              {{ request()->is('TeachingStaff') || request()->is('NonTeachingStaff') ? 'text-red-700' : '' }}" aria-label="Staff">
        <span>Staff</span>
        <span class="dropdown-symbol 
              {{ request()->is('TeachingStaff') || request()->is('NonTeachingStaff') ? 'text-red-700' : '' }}">
            &#9662;
        </span>
    </a>
    <div class="sub-menu1 absolute hidden bg-white shadow-lg z-50" id="sub-staff">
        <a href="{{ route('TeachingStaff') }}" 
           class="block px-4 py-2 hover:bg-gray-100 
                  {{ request()->is('TeachingStaff') ? 'text-red-700' : '' }}" aria-label="Teaching Staff">
            Teaching Staff
        </a>
        <a href="{{ route('NonTeachingStaff') }}" 
           class="block px-4 py-2 hover:bg-gray-100 
                  {{ request()->is('NonTeachingStaff') ? 'text-red-700' : '' }}" aria-label=" Non-Teaching Staff">
            Non-Teaching Staff
        </a>
    </div>
</li>
<li>
    <a href="{{ route('Events') }}" 
       class="text-lg hover:text-red-700 
              {{ request()->is('Events') ? 'text-red-700' : '' }}" aria-label="Events">
        Events
    </a>
</li>
<li>
    <a href="{{ route('Contact') }}" 
       class="text-lg hover:text-red-700 
              {{ request()->is('Contact') ? 'text-red-700' : '' }}" aria-label="Contact">
        Contact
    </a>
</li>
